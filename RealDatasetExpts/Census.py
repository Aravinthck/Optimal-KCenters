...
cols.extend(['y']) 
df_train = pd.read_csv(os.path.join(DATA_DIR, 'adult.data'), header=None)
df_train.columns = cols

df_test = pd.read_csv(os.path.join(DATA_DIR, 'adult.test'),skiprows=1, header=None) 

df_test.columns = cols

df_test.loc[df_test.y == ' <=50K.', 'y'] = ' <=50K'
df_test.loc[df_test.y == ' >50K.', 'y'] = ' >50K'
...
cat_col = ['X2','X4','X6','X7','X8','X9','X10','X14','y']
...
mmOpt.set_model(MinMax(initConstrCnt=20, optimalGap=0.01, tol=0.05, time= 5, outputFlag=False))
...
with open(os.path.join(RESULT_DIR, 'Census.pkl'), 'wb') as outp:
...
with open(os.path.join(DATAINFO_DIR, 'Census.pkl'), 'wb') as outp:

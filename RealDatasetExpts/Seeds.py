...
cols.extend('y')
df = pd.read_csv(os.path.join(DATA_DIR, 'seeds_dataset.txt'), delim_whitespace=True, header=None) 
# print(df.columns)
df.columns = cols
...
mmOpt.set_model(MinMax(initConstrCnt=13, optimalGap=0.01, tol=0.0005, time= 3))
...
with open(os.path.join(RESULT_DIR, 'seeds.pkl'), 'wb') as outp:
...
with open(os.path.join(DATAINFO_DIR, 'seeds.pkl'), 'wb') as outp:

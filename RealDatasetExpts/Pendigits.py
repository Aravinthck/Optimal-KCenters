...
cols.extend(['y'])
df_test = pd.read_csv(os.path.join(DATA_DIR, 'pendigits.tes'), header=None) 
df_train = pd.read_csv(os.path.join(DATA_DIR, 'pendigits.tra'), header=None) 
...
mmOpt.set_model(MinMax(initConstrCnt=30, optimalGap=0.05, tol=1, time= 5,outputFlag=False))
...
with open(os.path.join(RESULT_DIR, 'pendigits.pkl'), 'wb') as outp:
...
with open(os.path.join(DATAINFO_DIR, 'pendigits.pkl'), 'wb') as outp:

...
cols.extend(['y'])
df = pd.read_csv(os.path.join(DATA_DIR, 'codrna.csv'), header=None) 
df.columns = cols
...
mmOpt = UnsupervisedClustering(K = K, max_iter= 20, random_state=68)
mmOpt.set_model(MinMax(initConstrCnt=8, optimalGap=0.01, tol=1, time= 3, outputFlag=False))
...
with open(os.path.join(RESULT_DIR, 'Codrna.pkl'), 'wb') as outp:
...
with open(os.path.join(DATAINFO_DIR, 'codrna.pkl'), 'wb') as outp:

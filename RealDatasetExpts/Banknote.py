...
cols.extend('y')
df = pd.read_csv(os.path.join(DATA_DIR, 'data_banknote_authentication.txt'), header=None, sep = ',') 
df.columns = cols
...
mmOpt.set_model(MinMax(initConstrCnt=12, optimalGap=0.01, tol=0.005, time= 5, outputFlag=False))
...
with open(os.path.join(RESULT_DIR, 'Banknote.pkl'), 'wb') as outp:
...
with open(os.path.join(DATAINFO_DIR, 'banknote.pkl'), 'wb') as outp:

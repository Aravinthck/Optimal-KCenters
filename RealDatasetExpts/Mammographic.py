...
cols.extend('y')
df = pd.read_csv(os.path.join(DATA_DIR, 'mammographic_masses.data'), header=None, sep = ',') 
df.columns = cols
...
mmOpt.set_model(MinMax(initConstrCnt=20, optimalGap=0.01, tol=0.005, time= 5))
...
with open(os.path.join(RESULT_DIR, 'mammographic.pkl'), 'wb') as outp:
...
with open(os.path.join(DATAINFO_DIR, 'mammographic.pkl'), 'wb') as outp:

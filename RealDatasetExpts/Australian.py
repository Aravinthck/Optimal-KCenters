...
cols.extend(['y'])
df = pd.read_csv(os.path.join(DATA_DIR, 'australian.dat'), header=None, delim_whitespace=True) 
# print(df.columns)
df.columns = cols
...
print(df_data.shape, centers_kcgon.shape)
...
print(df_data.shape, centers_list_gonavg[0].shape)
...
print(df_data.shape, centers_kchs.shape)
...
print(df_data.shape, centers_list_km[0].shape)
...
print(df_data.shape, centers_list_kmplus[0].shape)

# k-MinMax 

mmOpt = UnsupervisedClustering(K = K, max_iter= 20, random_state=27)
mmOpt.set_model(MinMax(initConstrCnt=6, optimalGap=0.01, tol=0.05, time= 3, outputFlag=False))
...
print(df_data.shape, centers_mmopt.shape)


# Save the final results 


with open(os.path.join(RESULT_DIR, 'Australian.pkl'), 'wb') as outp:
...
with open(os.path.join(DATAINFO_DIR, 'Australian.pkl'), 'wb') as outp:

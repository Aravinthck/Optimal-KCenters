...
cols.extend('y')
df = pd.read_csv(os.path.join(DATA_DIR, 'column_3C.dat'), delim_whitespace=True, header=None) 
print(df.columns)
df.columns = cols
...
mmOpt = UnsupervisedClustering(K = K , max_iter= 20, random_state=54)
mmOpt.set_model(MinMax(initConstrCnt=6, optimalGap=0.05, tol=0.05, time= 3))
...
with open(os.path.join(RESULT_DIR, 'vertebral.pkl'), 'wb') as outp:
...
with open(os.path.join(DATAINFO_DIR, 'vertebral.pkl'), 'wb') as outp:

...
cols.extend('y')
df = pd.read_csv(os.path.join(DATA_DIR, 'Skin_NonSkin.txt'), header=None, delim_whitespace= True) 
df.columns = cols
...
mmOpt = UnsupervisedClustering(K = K , max_iter= 40, random_state=0)
mmOpt.set_model(MinMax(initConstrCnt=20, optimalGap=0.05, tol=0.5, time= 3))
...
with open(os.path.join(RESULT_DIR, 'skinsegment.pkl'), 'wb') as outp:
...
with open(os.path.join(DATAINFO_DIR, 'skinsegment.pkl'), 'wb') as outp:

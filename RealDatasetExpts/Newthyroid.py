...
K = 3
cols = [ 'X' + str(i+1) for i in range(5) ]
cols.extend('y')
df = pd.read_csv(os.path.join(DATA_DIR, 'newthyroid.dat'), skiprows=10, header = None)
...
mmOpt.set_model(MinMax(initConstrCnt=5, optimalGap=0.0,tol=0.005, time = 1))
...
with open(os.path.join(RESULT_DIR, 'Newthyroid.pkl'), 'wb') as outp:
...
with open(os.path.join(DATAINFO_DIR, 'Newthyroid.pkl'), 'wb') as outp:

...
cols = ['y']
cols.extend([ 'X' + str(i+1) for i in range(f) ])
df = pd.read_csv(os.path.join(DATA_DIR, 'balance-scale.data'), header=None) 
# print(df.columns)
df.columns = cols
...
mmOpt = UnsupervisedClustering(K = K , max_iter= 40, random_state=234)
mmOpt.set_model(MinMax(initConstrCnt=30, optimalGap=0.05, tol=0.1, time= 45, outputFlag=False))
...
with open(os.path.join(RESULT_DIR, 'balance.pkl'), 'wb') as outp:
...
with open(os.path.join(DATAINFO_DIR, 'balance.pkl'), 'wb') as outp:

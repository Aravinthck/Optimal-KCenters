...
df1 = pd.read_csv(os.path.join(DATA_DIR, 'sepsis_survival_primary_cohort.csv'),  sep = ',') 
df2 = pd.read_csv(os.path.join(DATA_DIR, 'sepsis_survival_study_cohort.csv'),  sep = ',') 
df3 = pd.read_csv(os.path.join(DATA_DIR, 'sepsis_survival_validation_cohort.csv'), sep = ',') 
...
mmOpt = UnsupervisedClustering(K = K , max_iter= 25, random_state=234)
mmOpt.set_model(MinMax(initConstrCnt=10, optimalGap=0.01, tol=0.005, time= 5, outputFlag=False))
...
with open(os.path.join(RESULT_DIR, 'sepsis.pkl'), 'wb') as outp:
...
with open(os.path.join(DATAINFO_DIR, 'sepsis.pkl'), 'wb') as outp:

...
X = wine.data 
y = wine.target
cols = columns = [ 'X' + str(i + 1) for i in range(X.shape[1]) ]
...
mmOpt = UnsupervisedClustering(K =K , max_iter= 10, random_state=0)
mmOpt.set_model(MinMax(initConstrCnt=5, optimalGap=0.01, tol=0.5, outputFlag=False ))
...
with open(os.path.join(RESULT_DIR, 'Wine.pkl'), 'wb') as outp:
...
with open(os.path.join(DATAINFO_DIR, 'Wine.pkl'), 'wb') as outp:

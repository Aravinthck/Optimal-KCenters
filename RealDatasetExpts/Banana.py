...
cols.extend(['y'])
df = pd.read_csv(os.path.join(DATA_DIR, 'banana.dat'), skiprows = 7, header=None) 
...
mmOpt.set_model(MinMax(initConstrCnt=10, optimalGap=0.01, tol=0.05, time= 5, outputFlag=False))
...
with open(os.path.join(RESULT_DIR, 'banana.pkl'), 'wb') as outp:
...
with open(os.path.join(DATAINFO_DIR, 'banana.pkl'), 'wb') as outp:

...
cols = [ 'X' + str(i) for i in range(f+1) ]
cols.extend('y')
df = pd.read_csv(os.path.join(DATA_DIR, 'ecoli.data'), header=None, delim_whitespace= True) 
df.columns = cols
...
X=df.iloc[:,1:f+1]
df_data = df.copy()
df_data = df_data.drop(['X0','y'], axis=1)
...
mmOpt = UnsupervisedClustering(K = K , max_iter= 20, random_state=0)
mmOpt.set_model(MinMax(initConstrCnt=10, optimalGap=0.05, tol=0.1, time= 40, outputFlag=False))
...
with open(os.path.join(RESULT_DIR, 'ecoli.pkl'), 'wb') as outp:
...
with open(os.path.join(DATAINFO_DIR, 'ecoli.pkl'), 'wb') as outp:

...
cols.extend('y')
df_test = pd.read_csv(os.path.join(DATA_DIR, 'shuttle.tst'), header=None, delim_whitespace = True) 
df_train = pd.read_csv(os.path.join(DATA_DIR, 'shuttle.trn'), header=None, delim_whitespace = True) 
...
mmOpt.set_model(MinMax(initConstrCnt=8, optimalGap=0.05, tol=0.5, time= 30, outputFlag=False))
...
with open(os.path.join(RESULT_DIR, 'shuttle.pkl'), 'wb') as outp:
...
with open(os.path.join(DATAINFO_DIR, 'shuttle.pkl'), 'wb') as outp:

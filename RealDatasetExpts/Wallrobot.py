...
cols.extend('y')
df = pd.read_csv(os.path.join(DATA_DIR, 'sensor_readings_4.data'), header=None) 
df.columns = cols
...
mmOpt.set_model(MinMax(initConstrCnt=5, optimalGap=0.01, tol=0.05, time= 3))
...
with open(os.path.join(RESULT_DIR, 'wallrobot.pkl'), 'wb') as outp:
...
with open(os.path.join(DATAINFO_DIR, 'wallrobot.pkl'), 'wb') as outp:
